...
object EventFilter extends App with EventMarshalling {
  ...
  // runMain org.jordimasip.stream.EventFilter C:\Users\Jordi\akkaProjectes\provaStreams\textEntradaEvent.txt C:\Users\Jordi\akkaProjectes\provaStreams\textSortidaEvent.txt error

  val config = ConfigFactory.load()
  val maxLine = config.getInt("log-stream-processor.max-line")

  if (args.length != 3) {
    System.err.println("Falten arguments: input-file output-file state")
    System.exit(1)
  }
